...
class OperationBase extends WorkloadModuleBase {
    ...
    createConnectorRequest(operation, args, contract = 'databaseTransaction') {
        switch (this.connectorType) {
            case 'ethereum':
                return this._createEthereumConnectorRequest(operation, args, contract);
            default:
                throw new Error(`Connector type ${this.connectorType} is not supported by the benchmark`);
        }
    }

    _createEthereumConnectorRequest(operation, args, contract = 'databaseTransaction') {
        return {
            contract: contract,
            verb: operation,
